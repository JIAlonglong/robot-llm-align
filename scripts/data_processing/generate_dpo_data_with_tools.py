#!/usr/bin/env python3
"""
DPO 偏好数据生成 - 增强版（带工具调用）
用 DeepSeek-V3.2 生成 500 个问题 + chosen（可能包含工具调用）
"""
...
OUTPUT   = "dataset/dpo_chosen_with_tools.jsonl"
...
**路径规划**：
- rrt_planning(start_x, start_y, goal_x, goal_y, obstacle_list) - RRT 随机树路径规划，返回路径点和长度
- astar_planning(start_x, start_y, goal_x, goal_y, grid_size, robot_radius, obstacle_list) - A* 网格路径规划
- cubic_spline_planning(waypoints) - 三次样条曲线轨迹生成，waypoints格式 "x1,y1;x2,y2;..."

**控制**：
- simulate_pid(kp, ki, kd, setpoint, duration) - PID 控制仿真，返回超调量/稳态误差/调节时间
- lqr_steering_control(x, y, yaw, v, ref_path) - LQR 转向控制
- mpc_control(x, y, yaw, v, ref_path, horizon) - MPC 预测控制

**定位与状态估计**：
- ekf_localization(state, control, measurement) - 扩展卡尔曼滤波，state格式 "x,y,yaw"
- particle_filter_localization(initial_state, measurements, num_particles) - 粒子滤波定位

**机械臂**：
- arm_forward_kinematics(joint_angles, link_lengths) - 机械臂正运动学，joint_angles格式 "θ1,θ2,θ3"（弧度）

**可视化**：
- plot_path_comparison(paths, labels, title) - 多路径对比图
...
# 各主题配置
TOPICS = [
    {"name": "reinforcement_learning", "desc": "强化学习：MDP、值函数、策略梯度、Q-learning、Actor-Critic、PPO、SAC、奖励设计", "count": 120, "chapters": RL_CHAPTERS, "tool_ratio": 0.4},
    {"name": "pid_control",            "desc": "PID控制：比例积分微分原理、参数整定（Z-N法）、稳定性分析、抗积分饱和、工程应用", "count": 80,  "chapters": CONTROL_CHAPTERS, "tool_ratio": 0.5},
    {"name": "mpc",                    "desc": "模型预测控制MPC：滚动优化、约束处理、终端条件、稳定性、非线性MPC、经济MPC", "count": 80,  "chapters": CONTROL_CHAPTERS, "tool_ratio": 0.5},
    {"name": "robot_kinematics",       "desc": "机器人运动学与动力学：正逆运动学、DH参数、雅可比矩阵、轨迹规划、力控制", "count": 80,  "chapters": ROBOTICS_CHAPTERS, "tool_ratio": 0.3},
    {"name": "robot_learning",         "desc": "机器人学习：模仿学习、sim-to-real迁移、多任务RL、课程学习、机器人抓取", "count": 80,  "chapters": ROBOTICS_CHAPTERS, "tool_ratio": 0.4},
    {"name": "general_control",        "desc": "通用控制理论：状态空间、李雅普诺夫稳定性、鲁棒控制、自适应控制、卡尔曼滤波", "count": 60,  "chapters": CONTROL_CHAPTERS, "tool_ratio": 0.3},
]
...
def generate_questions_with_context(client, topic: Dict, context: str, chapter_name: str, batch_size: int, need_tool: bool) -> List[str]:
    """基于教材上下文生成问题"""

    tool_hint = ""
    if need_tool:
        tool_hint = "\n- 至少一半的问题应该是需要仿真、计算、绘图等操作的（这些问题回答时会用到工具）"
...
要求：
- 如果问题需要仿真、计算、绘图等操作，在回答中使用 <tool_call>function_name(params)</tool_call> 调用工具
- 如果是纯理论问题，直接回答即可，不需要工具
...
def main():
    ...
    for topic in TOPICS:
        ...
        print(f"\n{'='*60}")
        print(f"主题: {topic['name']} | 已有: {already} | 还需: {needed} | 工具比例: {topic['tool_ratio']:.0%}")
        print(f"{'='*60}")
        ...
        with tqdm(total=needed, desc=topic["name"]) as pbar:
            while count < needed:
                ...
                # 随机决定这批问题是否需要工具
                import random
                need_tool = random.random() < topic["tool_ratio"]

                context, chapter_name = get_context(topic["chapters"])
                questions = generate_questions_with_context(client, topic, context, chapter_name, batch, need_tool)
    ...
    with open(OUTPUT, "r", encoding="utf-8") as f:
        for line in f:
            if json.loads(line).get("has_tool"):
                tool_count += 1
    print(f"📊 工具调用数据: {tool_count}/{total} ({tool_count/total:.1%})")
